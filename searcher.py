...
class BFSearcher(Searcher): 
    ...
    """
    def next_state(self):
        """ gives the state that has been the list the longest
        """
        s = self.states[0]
        self.states.remove(s)
        return s 
...
class DFSearcher(Searcher): 
    ...
    """
    def next_state(self):
        """ gives the state that has been the list the shortest 
        """
        s = self.states[-1]
        self.states.remove(s)
        return s 
...
def h2(state):
    """ a heuristic that returns a more accurate estimate of the number of moves
        required to get from the state to the goal state. 
    """
    return state.board.rc_misplaced()
...
class GreedySearcher(Searcher):
    ...
    def next_state(self):
        """ chooses one of the states with the highest priority to be processed. """
        s = max(self.states)
        self.states.remove(s)
        return s[1]
